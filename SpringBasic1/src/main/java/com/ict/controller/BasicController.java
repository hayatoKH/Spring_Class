...
public class BasicController {
	...
	// 만약 주소와 매칭된 메서드와 리턴자료형을 String이 아닌 void로 처리하는 경우
	// 지정주소.jsp로 바로 연결굅니다.(view파일(.jsp) 이름 지정 불가)
	// 주소와 파일명이 일치한다면 써주셔도 되지만
	// 기본적으로는 String을 쓰는 것을 추천드립니다.
	@GetMapping("/test") // test.jsp로 바로 연결됨
	public void goTest() {
		//내부 실행문 없음.
	}
	
	// VO을 활용해 회원 데이터를 받는 컨트롤러를 만들어보겠습니다.
	// /userInfo가 주소입니다.
	@GetMapping("/userInfo")
	public String getUserInfo(UserVO userVO, Model model) {
		// 변수명은 userVO로 지정 했으나, 실제로는 내부 멤버변수의 이름으로 데이터를 받습니다.
